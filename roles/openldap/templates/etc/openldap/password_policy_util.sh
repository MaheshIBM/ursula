...
if [ $check_user_name ]; then
  ...
  dn=`ldapsearch -LLL -x -D {{openldap.openldap_user_dn}} -b {{openldap.organizationalUnit}} -w {{openldap.openldap_user_password}} -s sub sn="$user_name" -o ldif-wrap=no | head -1`
...
if [ ${options['s']} ]; then
   echo "Show password policy for user $user_name"
   pwdPolicySubentry=`ldapsearch -LLL -x -D {{openldap.openldap_user_dn}} -b {{openldap.organizationalUnit}} -w {{openldap.openldap_user_password}} -s sub sn="$user_name" -o ldif-wrap=no + | grep pwdPolicySubentry`
   if [ "$pwdPolicySubentry" = "pwdPolicySubentry: {{openldap.password_nopolicy_dn}}" ]; then
      echo "Password policy does not apply for $user_name"
   else
      ldapsearch -LLL -x -D{{openldap.openldap_user_dn}} -b {{openldap.password_policy_dn}} -w {{openldap.openldap_user_password}} -s sub
   fi 
fi
